...
export default function ServiceUi({ title, description, items }) {
  ...
  return (
    <div ref={serviceUi} className="  mt-10 grid grid-cols-1 gap-5 md:grid-cols-2 md:gap-24">
      ...
      <div ref={services}  className=" select-none leading-[2.5rem] text-secondary-500 md:leading-[2.5rem] lg:leading-[3.4rem]  translate-y-10 opacity-0">
        {items.map((item, index) => {
          return (
            <p key={index} className="font-general  2xl:text-7xl font-extrabold  text-[48px]">
              {item}
            </p>
          );
        })}
